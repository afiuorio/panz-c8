...
void execute8XY4(WORD opcode, c8_cpu *cpu) {
    ...
    WORD value = cpu->registers[fir] + cpu->registers[sec];
    if (value > 255)
        cpu->registers[15] = 1;
...
void executeCXNN(WORD opcode, c8_cpu *cpu) {
    ...
    cpu->registers[fir] = cpu->randomByte & mask; // TODO: stub, should be
                                                  // random
}
...
void executeDXYN(WORD opcode, c8_cpu *cpu) {
    ...
    BYTE coordx = cpu->registers[fir];
    BYTE coordy = cpu->registers[sec];
